# Comparison Result of './output/test_translated_sqlglot_high_school_dataset.json'
...
```mysql
SELECT c.course_name, t.teacher_name FROM courses AS c JOIN teachers AS t ON c.teacher_id = t.teacher_id
```
...
```mysql
SELECT s.student_name, s.class_name FROM students AS s WHERE s.class_name = 'Class 04'
```
...
```mysql
SELECT student_id, student_name, class_name FROM (SELECT student_id, student_name, class_name, ROW_NUMBER() OVER (PARTITION BY class_name ORDER BY CASE WHEN student_id IS NULL THEN 1 ELSE 0 END, student_id) AS rn FROM students) AS sub WHERE rn = 1
```
...
```mysql
SELECT T.teacher_name, C.course_name FROM teachers AS T JOIN LATERAL (SELECT course_name, course_id FROM courses WHERE teacher_id = T.teacher_id ORDER BY CASE WHEN course_id IS NULL THEN 1 ELSE 0 END DESC, course_id DESC LIMIT 1) AS C ON TRUE
```
...
```mysql
SELECT student_id, student_name, class_name FROM (SELECT student_id AS student_id, student_name AS student_name, class_name AS class_name, ROW_NUMBER() OVER (PARTITION BY class_name ORDER BY CASE WHEN class_name IS NULL THEN 1 ELSE 0 END, class_name, CASE WHEN student_id IS NULL THEN 1 ELSE 0 END, student_id) AS _row_number FROM students) AS _t WHERE _row_number = 1
```
...
```mysql
SELECT REGEXP_REPLACE(course_name, '\\d+$', '') AS base_course_name, COUNT(*) AS course_count FROM courses GROUP BY base_course_name ORDER BY CASE WHEN course_count IS NULL THEN 1 ELSE 0 END DESC, course_count DESC
```
...
```mysql
ALTER TABLE students ADD PRIMARY KEY (CASE WHEN student_id IS NULL THEN 1 ELSE 0 END, student_id)
```

--- Error Message ---
(1064, "You have an error in your SQL syntax; check the manual that corresponds to your MySQL server version for the right syntax to use near 'CASE WHEN student_id IS NULL THEN 1 ELSE 0 END, student_id)' at line 1")
...
--- Error Message ---
(1064, "You have an error in your SQL syntax; check the manual that corresponds to your MySQL server version for the right syntax to use near 'RETURNING course_id' at line 1")
...
```mysql
SELECT t.teacher_name, JSONB_AGG(JSONB_BUILD_OBJECT('course_id', c.course_id, 'course_name', c.course_name)) AS courses_taught FROM teachers AS t JOIN courses AS c ON t.teacher_id = c.teacher_id GROUP BY t.teacher_name
```

--- Error Message ---
(1305, 'FUNCTION high_school.JSONB_AGG does not exist')
...
```mysql
SELECT t.teacher_name, COALESCE(GROUP_CONCAT(c.course_name SEPARATOR ', '), '无课程') AS courses FROM teachers AS t LEFT JOIN courses AS c ON t.teacher_id = c.teacher_id GROUP BY t.teacher_name
```
...
```mysql
SELECT student_name, class_name FROM students WHERE REGEXP_LIKE(student_name, '王')
```
...
```mysql
WITH ClassStudents AS (SELECT class_name, GROUP_CONCAT(student_name ORDER BY CASE WHEN student_name IS NULL THEN 1 ELSE 0 END, student_name) AS student_names FROM students GROUP BY class_name) SELECT * FROM ClassStudents WHERE '王丹' = ANY(student_names)
```

--- Error Message ---
(1064, "You have an error in your SQL syntax; check the manual that corresponds to your MySQL server version for the right syntax to use near 'student_names)' at line 1")
...
LINE 1: REPLACE INTO courses (course_id, course_name, teacher_id) VA...
        ^
...
```postgresql
SELECT t.teacher_name, STRING_AGG(c.course_name, ' | ') AS courses_taught FROM teachers AS t LEFT JOIN courses AS c ON t.teacher_id = c.teacher_id GROUP BY t.teacher_name
```
...
```postgresql
SELECT student_id, student_name, class_name FROM students LIMIT 5 OFFSET 10
```
...
--- Error Message ---
function find_in_set(text, unknown) does not exist
LINE 1: ...LECT student_name, class_name FROM students WHERE FIND_IN_SE...
                                                             ^
